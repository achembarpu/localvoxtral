...
/// `/v1/realtime` on the same port, driving a `SpeechASRStreamingSession` per connection.
///
...
public final class RealtimeSpeechServer: @unchecked Sendable {
    ...
    /// executable target, which cannot see the (module-internal) model types. Sets the MLX
    /// GPU cache limit and loads from an HF id or a local directory. The engine is inferred
    /// from `modelID` (see `SpeechASREngineKind.infer`).
    ...
    private func dispatch(_ message: RealtimeClientMessage, _ connection: NWConnection, _ ctx: Connection) {
        inferenceQueue.async { [weak self] in
            ...
            switch message {
            case .sessionUpdate(let delivery):
                // The app only requests revisions for a capable model and an Overlay
                // session. Keeping negotiation session-scoped avoids expensive
            ...
            case .audioAppend(let base64):
                ...
                for batch in ctx.stepBatcher.append(samples) {
                    ...
                    session.step(batch)
                    if ctx.transcriptDelivery == .revisableSnapshot {
                        self.emitTranscriptUpdate(session.text, connection, ctx)
                        continue
                    }
    ...
    private func ensureSession(_ ctx: Connection) -> SpeechASRStreamingSession {
        ...
        let s = engine.makeSession(
            transcriptionDelayMs: transcriptionDelayMs, transcriptDelivery: ctx.transcriptDelivery
        )
        ctx.session = s
        return s
    }

    /// Emits no frame when a full model snapshot has not changed. This keeps the
    /// revision path's allocation, JSON, socket, and overlay work proportional to useful
    /// transcript changes rather than audio step cadence.
...
enum SpeechModelLoader {
    static func load(
        ...
        case .voxtral:
            ...
            return VoxtralASREngine(model: model)
        case .nemotron:
            let model: NemotronASRModel
            if let dir = modelDirectory {
                model = try NemotronASRModel.fromDirectory(URL(fileURLWithPath: dir))
            } else if let id = modelID, let revision = modelRevision {
                let directory = try SpeechHFCacheModelLocator.locate(
                    repoID: id,
                    revision: revision,
                ...
                model = try await NemotronASRModel.fromPretrained(id)
            } else {
                throw RealtimeSpeechServer.ServerError.noModelSpecified
            }
